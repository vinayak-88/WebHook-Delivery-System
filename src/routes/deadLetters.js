...
// POST /dead-letters/replay-all - Bulk replay all waiting DLQ jobs
router.post('/replay-all', async (req, res) => {
  ...
  try {
    const jobs = await deadLetterQueue.getJobs(['waiting'], 0, BATCH_SIZE - 1, false);
    const totalFound = jobs.length;

    for (const deadLetterJob of jobs) {
      try {
        ...
        logger.error('Failed to replay DLQ job in bulk batch', {
    ...
    logger.info('Bulk DLQ replay completed', {
      found: totalFound,
      replayed,
      skipped,
      failed,
    ...
    logger.error('Failed to process bulk DLQ replay', { error: err.message });
    return res.status(500).json({ error: 'Internal server error' });
  }
});
